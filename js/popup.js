  ...
  for (line of lines) {
    ...
    switch (title) {
      ...
      case "NSS":
        ...
        break;
      case "TEL":
        current_entry.telephone = data;
        break;
      case "CAP":
        current_entry.captcha = data;
        break;
...
chrome.storage.session.get(["entries"], (result) => {
  if (result.entries === undefined) return;
...
start_all_btn.onclick = (e) => {
  for (entry of entries) if (!entry.running) StartTask(entry);
};
stop_all_btn.onclick = (e) => {
  for (entry of entries) if (entry.running) StopTask(entry);
};
...
function UpdateUI() {
  ...
  for (const [i, entry] of entries.entries()) {
    ...
    if (entry.success) name_state_class = "success";
    else if (!entry.running) name_state_class = "stopped";
    ...
    if (entry.running) atleast_one_running = true;
    else atleast_one_stopped = true;

    // events
    view_btn.onclick = (e) =>
      chrome.tabs.update(entry.tab_id, { selected: true });
    start_btn.onclick = (e) => StartTask(entry);
    stop_btn.onclick = (e) => StopTask(entry);
  }

  // stop/start all buttons
  if (atleast_one_running) stop_all_btn.disabled = false;
  else stop_all_btn.disabled = true;

  if (atleast_one_stopped) start_all_btn.disabled = false;
  else start_all_btn.disabled = true;
}
...
document.getElementsByTagName("svg")[0].onclick = (e) => {
  console.log("dd");
  chrome.tabs.create({ url: "https://www.github.com/napacodes" });
};
